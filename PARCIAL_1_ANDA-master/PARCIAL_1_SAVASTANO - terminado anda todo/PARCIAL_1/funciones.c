...
int getEmail(char email[], int cantidad)
{
    ...
    getString("Ingrese su mail : ",auxiliar);
    //printf(" su mail es : %s", auxiliar);
...
void ponerMayusculas (char stringAUsar[])
    {
        int cantidadLetras;
        int i;

        cantidadLetras=strlen(stringAUsar);
        strlwr(stringAUsar);

        for(i=0;i<cantidadLetras;i++)
        {
            if((i==0) || (stringAUsar[i-1]==' '))
            {
                if(stringAUsar[i]>='a' && stringAUsar[i]<='z')
                {
                    stringAUsar[i]=stringAUsar[i]-'a'+'A';
                }//if(stringAUsar[i]>'a' && stringAUsar[i]<'z')
            }//if((i==0) || (stringAUsar[i-1]==' '))
        }//for(i=0;i<stringAUsar;i++)
    }



int validSexo(char sexo)
{
  int validMasc=0;
  int validFem=0;
